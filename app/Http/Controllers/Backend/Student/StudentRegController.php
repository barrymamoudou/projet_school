...
class StudentRegController extends Controller
{
    ...
    public function StudentYearClassWise(Request $request){
        ...
        $data['alldata']= AssignStudent::where('year_id', $request->year_id)->where('class_id',$request->year_id)->get();
        return view('backend.student.student_reg.student_view', $data);
    }
    ...
    public function StudentRegStore(Request $request){
        DB::transaction(function() use ($request){
            //ici on recupere l'anne l'id de l'anne en question qui correspond au formulaire
            $checkAnne=StudentYear::find($request->year_id)->name;
            $student = User::where('usertype','Student')->orderBy('id','DESC')->first();

            if($student ==null){
                $firstReg=0; // le premier element 
